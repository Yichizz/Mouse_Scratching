...
class ScratchDetector:
    ...
    def plot_results(self, r, keypoints):
        ...
        right_angle = self.compute_angle(
            keypoints[0].reshape(1, 2),
            keypoints[5].reshape(1, 2),
            keypoints[3].reshape(1, 2),
        )
        left_angle = self.compute_angle(
            keypoints[0].reshape(1, 2),
            keypoints[5].reshape(1, 2),
            keypoints[4].reshape(1, 2),
        )
    ...
    def plot_results_together(self, r, keypoints):
        ...
        if len(self.scratchings["end"]) > 0 and len(self.scratchings["start"]) == len(
            self.scratchings["end"]
        ):
            self.ax3.clear()
            self.ax3.axis("off")
            self.tabledata = [
            ...
            for i in range(len(self.scratchings["start"])):
                start_min, start_sec, start_frame = self.frame_index2time(
                    self.scratchings["start"][i]
                )
                end_min, end_sec, end_frame = self.frame_index2time(
                    self.scratchings["end"][i]
                )
                duration_min, duration_sec, duration_frame = self.frame_index2time(
                    self.scratchings["duration"][i]
                )
                gap_min, gap_sec, gap_frame = self.frame_index2time(
                    self.scratchings["gaps"][i]
                )
                # add one more row to the table
                self.tabledata.append(
                    [
                        i + 1,
                        f"{start_min}:{start_sec}:{start_frame}",
                        f"{end_min}:{end_sec}:{end_frame}",
                        f"{duration_min}:{duration_sec}:{duration_frame}",
                        f"{gap_min}:{gap_sec}:{gap_frame}",
                        self.scratchings["side"][i],
                        self.scratchings["times"][i],
                        self.scratchings["intensity"][i],
                    ]
                )
            self.tabledata.extend(
                [self.blank_row for _ in range(24 - len(self.scratchings["start"]))]
            )
            self.ax3.table(
                cellText=self.tabledata, cellLoc="center", loc="center", fontsize=12
            )
        self.fig.canvas.draw()
        self.fig.canvas.flush_events()
    ...
    def compute_angle(self, p1, p2, p3):
        ...
        ), f"Expected same shape, got {p1.shape}, {p2.shape}, {p3.shape}"
        v1 = p1 - p2
        v2 = p3 - p2
        angle = np.arccos(
            np.sum(v1 * v2, axis=1)
    ...
    def evaluate_scratching(self):
        ...
        licking_index = np.where(np.array(self.behaviours_scratching) == "paw_licking")[
            0
        ]
        keypoints_scratching = np.array_split(keypoints_array, licking_index)
        keypoints_scratching = [
            keypoints for keypoints in keypoints_scratching if keypoints.shape[0] > 1
        ]

        # Calculate average speed for right and left limbs
        speed_right = [
            np.mean(np.linalg.norm(np.diff(scratch[:, 1], axis=0), axis=1))
            for scratch in keypoints_scratching
        ]
        speed_left = [
            np.mean(np.linalg.norm(np.diff(scratch[:, 2], axis=0), axis=1))
            for scratch in keypoints_scratching
        ]
        side = [
            "right" if speed_right[i] > speed_left[i] else "left"
            for i in range(len(keypoints_scratching))
        ]

        # Calculate the angles for the side of scratching
        angles = [
            self.compute_angle(
                scratch[:, 0], scratch[:, 3], scratch[:, 1 if side[i] == "right" else 2]
            )
            for i, scratch in enumerate(keypoints_scratching)
        ]
        times = np.sum(
            [self.count_local_minima(angle, threshold=45) for angle in angles]
        )
    ...
    def detect_scratching(self, class_highest_conf, keypoints):
        ...
        if class_highest_conf == 1 and len(self.scratchings["start"]) == len(
            self.scratchings["end"]
        ):
            gap = (
                self.i - self.scratchings["end"][-1]
        ...
        elif class_highest_conf == 2 and len(self.scratchings["start"]) == len(
            self.scratchings["end"]
        ):
            gap = (
                self.i - self.scratchings["end"][-1]
            ...
            if gap <= self.fps * 0.1:
                ...
                self.scratchings["gaps"].append(gap)
                self.keypoints_scratching.append(keypoints[[0, 3, 4, 5]])
                self.behaviours_scratching.append("paw_licking")
            else:
                # when the gap is large than 0.1 seconds, it's not a continuous scratching event
                # ignore the current paw licking event, since we don't start with paw licking
                self.is_continue = False

        # continue scratching when the class with the highest confidence is scratching or paw licking
        elif (class_highest_conf == 1 or class_highest_conf == 2) and len(
            self.scratchings["start"]
        ) > len(self.scratchings["end"]):
            self.scratchings["duration"][-1] += 1
            self.keypoints_scratching.append(keypoints[[0, 3, 4, 5]])
            self.behaviours_scratching.append(
                "scratching" if class_highest_conf == 1 else "paw_licking"
            )
        # end scratching when the class with the highest confidence is not scratching
        elif class_highest_conf == 0 and len(self.scratchings["start"]) > len(
            self.scratchings["end"]
        ):
            ...
            if self.validate_scratching() and times > 0:
                self.scratchings["side"].append(
                    side[0] if len(set(side)) == 1 else "both"
                )
                ...
                start_min, start_sec, start_frame = self.frame_index2time(
                    self.scratchings["start"][-1]
                )
                end_min, end_sec, end_frame = self.frame_index2time(
                    self.scratchings["end"][-1]
                )
                duration_min, duration_sec, duration_frame = self.frame_index2time(
                    self.scratchings["duration"][-1]
                )
                gap_min, gap_sec, gap_frame = self.frame_index2time(
                    self.scratchings["gaps"][-1]
                )
                with open(self.save_path, "a", newline="") as f:
                    f.write(
                        f'{len(self.scratchings["start"])},{start_min}:{start_sec}:{start_frame},{end_min}:{end_sec}:{end_frame},'
                    )
                    f.write(
                        f"{duration_min}:{duration_sec}:{duration_frame},{gap_min}:{gap_sec}:{gap_frame},"
                    )
                    f.write(
                        f'{self.scratchings["side"][-1]},{self.scratchings["times"][-1]},{self.scratchings["intensity"][-1]}\n'
                    )
            else:
    ...
    def merge_scratching(self):
        ...
        self.scratchings["intensity"][-2] = int(
            (self.scratchings["times"][-1] / self.scratchings["duration"][-1]) * 100
        )
    ...
    def process_video(self):
        ...
        while self.i < self.num_frames:
            ...
            # if no detection , skip the frame
            if len(r) == 0:
                min, sec, frame = self.frame_index2time(self.i)
                print(f"No detection of mouse at {min}:{sec}:{frame}")
                continue

            # Clear GPU memory if using CUDA
            if self.device.type == "cuda":
                torch.cuda.empty_cache()
            classes, classes_conf, points, points_conf = (
                r.boxes.cls,
                r.boxes.conf,
                r.keypoints.xy,
                r.keypoints.conf,
