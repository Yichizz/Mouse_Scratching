...
def reduce_redundancy(train_images, train_labels):
    ...
    np.random.seed(0)
    class_0_images_to_remove = np.random.choice(
        class_0_images, len(class_0_images) - max_count, replace=False
    )
    print(
        f"Number of images in class 0 after reduction: {len(class_0_images) - len(class_0_images_to_remove)}"
    )
