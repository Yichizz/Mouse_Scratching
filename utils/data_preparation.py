...
def json_to_yolo_keypoints(frames_path, labels_path, training_videos, testing_videos, val_frac = 0.1, verbose = True):
    '''
    Convert json labels to txt labels for YOLOv8 training and randomly split into train and val
    Args:
    ...
    if len(testing_videos) > 0:
        if os.path.exists('datasets/images/test') and os.path.exists('datasets/labels/test'):
        ...
        for sub_dir in os.listdir(frames_path):
            if sub_dir in testing_videos:
                ...
                for frame in tqdm(frames):
                    ...
                    txt_file = os.path.join('datasets', 'labels', 'test', f'{sub_dir}_{index:05d}.txt')
                    json_write_to_txt(json_file, txt_file, 6)
                    shutil.copy(os.path.join(frames_path, sub_dir, frame), os.path.join('datasets', 'images', 'test', f'{sub_dir}_{index:05d}.png'))
        print('Testing data converted to YOLO format')

    if os.path.exists('datasets/images/train') and os.path.exists('datasets/labels/train'):
    ...
    for sub_dir in os.listdir(frames_path):
        if sub_dir in training_videos:
            ...
            val_frames = np.random.choice(frames, int(len(frames) * val_frac), replace=False)
            ...
            for split, frames in zip(['train', 'val'], [train_frames, val_frames]):
                for frame in tqdm(frames):
                    index = int(frame.split('.')[0].split('img')[1])
                    json_file = os.path.join(labels_path, sub_dir, f'img{index:05d}.json')
                    txt_file = os.path.join('datasets', 'labels', split, f'{sub_dir}_{index:05d}.txt')
                    json_write_to_txt(json_file, txt_file, 6)
                    shutil.copy(os.path.join(frames_path, sub_dir, frame), os.path.join('datasets', 'images', split, f'{sub_dir}_{index:05d}.png'))
