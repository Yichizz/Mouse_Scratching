...
def pose2cls(
    ...
    label_dir,
    output_dir,
    cls=["mouse", "scratching_mouse", "paw_licking_mouse"],
):
    """
    ...
    for split in ["train", "val", "test"]:
        ...
        for l in tqdm(os.listdir(labels)):
            ...
            shutil.move(
                os.path.join(data, l.replace("txt", "png")),
                os.path.join(
                    output_dir, split, cls[cls_label], l.replace("txt", "png")
                ),
            )
    return None
...
def pose2detect(label_dir, background_dir=None, image_dir=None, val=0.2, test=0.1):
    ...
    if background_dir is not None:
        ...
        test_images = background_images[
            int(val * len(background_images)) : int(
                (val + test) * len(background_images)
            )
        ]
        ...
        for i in val_images:
            shutil.copy(
                os.path.join(background_dir, i), os.path.join(image_dir, "val", i)
            )
        for i in test_images:
            shutil.copy(
                os.path.join(background_dir, i), os.path.join(image_dir, "test", i)
            )
        for i in train_images:
            shutil.copy(
                os.path.join(background_dir, i), os.path.join(image_dir, "train", i)
            )
